...
import SearchedBooks from "../../Single/SearchedBooks/SearchedBooks";
import BookDetails from "../../Single/BookDetails/BookDetails";
import EditBook from "../../Single/EditBook/EditBook";

let BookSearchEngine = () => {

    const [ocClickForSearchedBooks , setOcClickForSearchedBooks] = useState(false);
    const [ocClickForBooksDetails , setOcClickForBooksDetails] = useState(false);
    const [ocClickForEditBook , setOcClickForEditBook] = useState(false);
    ...
    let handleSubmit = async(e) => {
        ...
        try {
            ...
            return setOcClickForSearchedBooks(!ocClickForSearchedBooks)
        }catch(err){
    ...
    return (
        <div className="book-search-engine the-service">
            ...
            <form action='' method='POST' onSubmit={handleSubmit} className="boo-search-form">
                ...
                {bookSearchEngineErrors && <small className='note'>{bookSearchEngineErrors.statusText}</small>}
                <input type="submit" className="submit" value="البحث"/>
            </form>

            {ocClickForSearchedBooks && <SearchedBooks 
            ocClickForSearchedBooks={() => setOcClickForSearchedBooks(!ocClickForSearchedBooks)} 
            ocClickForBooksDetails={() => setOcClickForBooksDetails(!ocClickForBooksDetails)}/>}

            {ocClickForBooksDetails && <BookDetails 
            ocClickForBooksDetails={() => {
                setOcClickForSearchedBooks(!ocClickForSearchedBooks);
                setOcClickForBooksDetails(!ocClickForBooksDetails);
            }} 
            ocClickForEditBook={() => {
                setOcClickForEditBook(!ocClickForEditBook);
                setOcClickForBooksDetails(!ocClickForBooksDetails);
            }} />}

            {ocClickForEditBook && <EditBook ocClickForEditBook={() => {
                setOcClickForEditBook(!ocClickForEditBook);
                setOcClickForBooksDetails(!ocClickForBooksDetails);
            }}/>}
